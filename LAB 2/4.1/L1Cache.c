...
uint32_t time;
Cache L1Cache; // Use the modified Cache structure for L1 cache
...
void accessDRAM(uint32_t address, uint8_t *data, uint32_t mode) {
    ...
    if (mode == MODE_READ) {
        memcpy(data, &(DRAM[address]), WORD_SIZE);
        time += DRAM_READ_TIME;
    }

    if (mode == MODE_WRITE) {
        memcpy(&(DRAM[address]), data, WORD_SIZE);
...
void accessL1(uint32_t address, uint8_t *data, uint32_t mode) {
    unsigned int cacheIndex, tag, offset, MemAddress;
    ...
    cacheIndex = (address / BLOCK_SIZE) & (L1_NUM_LINES-1);
    tag = address / L1_SIZE;
    ...
    if (!Line->Valid || Line->Tag != tag) { //if block not present
        ...
        if (Line->Valid && Line->Dirty) {
           MemAddress = (Line->Tag << 14) + (cacheIndex << 6);
